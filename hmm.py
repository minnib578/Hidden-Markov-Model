...
class ExactInference(Inference):
    ...
    def observe(self,observed_distances,landmarks):
        ...
        belief_container = [[0]*self.n_cols for _ in range(self.n_rows)]                                                # create empty probability container
        for index, mark in enumerate(landmarks):
            for r in range(self.n_rows):
                for c in range(self.n_cols):
                    point=self.get_coordinate(r,c)
                    dist=Euclidean_distance(point,mark)
                    if belief_container[r][c]==0:
                        belief_container[r][c]=normal_pdf(observed_distances[index],App.SENSOR_NOISE,dist)
                    else:
                        belief_container[r][c]=normal_pdf(observed_distances[index],App.SENSOR_NOISE,dist)*belief_container[r][c]
        belief_container=self.normalize(belief_container)                                                               # probability normalize
        for r in range(self.n_rows):                                                                                    # update belief table
            for c in range(self.n_cols):
                self.belief[r][c]=self.belief[r][c]*belief_container[r][c]                                              # p(x_t|e_{1:t})
        self.belief=self.normalize(self.belief)
    ...
    def timeUpdate(self):
        ...
        Hints:
          - p(x_{t+1}|e_{1:t}) = \sum_{x_t} p(x_{t+1}|x_t)p(x_t|e_{1:t}),
		    ...
		    the function self.transition_model(r, c) for the given x_t = (r, c).
        ...
        belief_container = [[0]*self.n_cols for _ in range(self.n_rows)]                                                # empty belief container
        for r in range(self.n_rows):
             for c in range(self.n_cols):
                 result=self.transition_model(r, c)                                                                     # transition model
                 for i in range(len(result)):
                     p,(_r,_c)=result[i]
                     belief_container[_r][_c]=self.belief[r][c]*p+belief_container[_r][_c]                              # update belief
        self.belief =self.normalize(belief_container)                                                                   #normalize belief
...
class ParticleFilter(Inference):
	...
	of the agent being in each cell.
    """
    NUM_PARTICLES = 500                                                                                                # total number of particles
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.particles = [None]*self.NUM_PARTICLES                                                                      # Initialize the distribution of particles randomly.
        for i in range(self.NUM_PARTICLES):                                                                             # Each element in self.particles represents a cell (r, c)
            r, c = random.randint(0, self.n_rows-1), random.randint(0, self.n_cols-1)
            self.particles[i]=(r,c)
        self.updateBelief()

    def updateBelief(self):                                                                                             # Convert the particle distributions to belief table
    ...
    def observe(self,observed_distances,landmarks):
        ...
        weight=[0]*self.NUM_PARTICLES                                                                                   # weight container
        for index, mark in enumerate(landmarks):
            for i,p in enumerate(self.particles):
                r,c=p
                point=self.get_coordinate(r,c)
                dist=Euclidean_distance(point,mark)
                if weight[i]==0:                                                                                        # calculate particles weight
                    weight[i]=normal_pdf(observed_distances[index],App.SENSOR_NOISE,dist)
                else:
                    weight[i]=normal_pdf(observed_distances[index],App.SENSOR_NOISE,dist)*weight[i]

        new_particles=random.choices(self.particles,weights=weight,k=self.NUM_PARTICLES)                                # according the weight of particles resampling
        self.particles=new_particles
        self.updateBelief()                                                                                             #update belief
    ...
    def timeUpdate(self):
        ...
        new_particles = [None]*self.NUM_PARTICLES                                                                       # new_particles container
        for index, p in enumerate(self.particles):                                                                      # acquire particle possible distribution
            next_state=self.transition_model(p[0],p[1])
            random_num=random.uniform(0, 1)
            p_pre=0
            for m in range (len(next_state)):                                                                           # determine next state for particles based random number
                p,point=next_state[m]
                if random_num >p_pre and random_num<p+p_pre:
                    new_particles[index]=point
                p_pre=p+p_pre
        self.particles=new_particles                                                                                    # new state particles
        self.updateBelief()                                                                                             # update the belief table based on the new particle distribution
